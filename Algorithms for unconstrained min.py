...
def fss_grad_descent(f, gradient_at_point, hessian_at_point, xk, step=None):
...
def oss_grad_descent(f, gradient_at_point, xk):
    ...
    if t_valid:
        ...
        fss_grad_descent(f, gradient_at_point, None, xk, t_optimal)
...
# Armijo-Goldstein-Wolfe method for minimum
def armijo_wolfe(gradient, xk, passo_iniziale, alfa=0.1, beta=0.7):
    print("\n3. Armijo-Goldstein-Wolfe method:")

def main():
    print(Fore.BLACK + Back.CYAN + "\nAlgorithms for unconstrained min: \ngradient descent (fixed and optimal step size), Newton's method, Armijo-Goldstein-Wolfe method\n" + Style.RESET_ALL)
    f = get_function()
    ...
    print("\n" + Fore.BLACK + Back.CYAN + "1. Gradient descent with fixed step size:" + Style.RESET_ALL)
    fss_grad_descent(f, gradient_at_point, hessian_at_point, xk)

    print("\n" + Fore.BLACK + Back.CYAN + "2. Gradient descent with optimal step size:" + Style.RESET_ALL)
    oss_grad_descent(f, gradient_at_point, xk)

    print("\n" + Fore.BLACK + Back.CYAN + "3. Newton's method for minimum:" + Style.RESET_ALL)
